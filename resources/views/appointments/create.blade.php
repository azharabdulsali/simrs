...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">New Appointment</h1>

    <form action="{{ route('appointments.store') }}" method="POST" class="space-y-4 max-w-md">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="datetime-local" name="scheduled_at" value="{{ old('scheduled_at') }}" class="w-full border px-2 py-1" required>
            @error('scheduled_at')<div class="text-red-600">{{ $message }}</div>@enderror
        </div>

        <div>
            <label class="block">Notes</label>
            <textarea name="notes" class="w-full border px-2 py-1">{{ old('notes') }}</textarea>
        </div>
